...
* Mirror "PENOR_right_main_UNMIRRORED.step" as you slice it, and make sure that it pauses on the layer just before the hole for the nut where the PCB will be screwed in gets covered, where you should add nuts into the two holes before resuming the print.
* Slice and print "PENOR_left_white.step", "PENOR_left_orange.step", and "PENOR_left_main.step" right after each other as shown in this video https://youtu.be/m_SXRGGoTQo, to make it possible to print the logos at the bottom with two different colors from the color used for the main part of the case. Remember to also add the pause for the nuts on the left half of the case too!
